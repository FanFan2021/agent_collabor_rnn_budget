...
class Graph(ABC):
    ...
    def construct_node_features(self) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        """
        Generates the 27-dim feature vector per agent.
        Optimized for 4/6 Byzantine Majority defense.
        """
        node_features = []
        rnn_hidden_states = []
        ...
        for idx in range(len(self.nodes)):
            ...
            # --- 1. SELF STATE & NORMALIZATION ---
            # solution: One-hot [5]
            solution = node.last_memory['solution_embed'].to(self.device).view(-1)
            # analysis: 384-dim normalized to unit sphere
            raw_analysis = node.last_memory['analysis_embed'].to(self.device).view(-1)
            analysis_embed = F.normalize(raw_analysis, p=2, dim=0)
            
            reliable_scores = node.last_memory['reliable_scores'].detach().to(self.device).view(-1)

            # --- 2. RNN MEMORY ---
            h_state = node.last_memory.get('rnn_hidden_states')
            if h_state is None:
            ...
            # --- 3. TEMPORAL PERSISTENCE (Against Coordinated Drift) ---
            ...
            # Categorical Persistence: 0 if flipped, small value if same
            sol_per = torch.norm(solution - first_sol, p=1).view(-1)
            
            # Semantic Persistence: Squared L2 Distance [0, 4]
            # Squaring punishes semantic 'pivoting' used by attackers to maintain consensus
            ana_per_dist = torch.norm(analysis_embed - first_ana, p=2).view(-1)
            ana_per = ana_per_dist ** 2 

            # --- 4. NEIGHBOR DYNAMICS ---
            neighbors_node_ids = node.last_memory.get('neighbors', [])
            ...
            if len(neighbors_node_ids) > 0:
                ...
                # Divergence Signals (Is this node an outlier?)
                sol_dev_norm = torch.norm(solution - neighbor_solution, p=1).view(-1)
                ana_dev_norm = torch.norm(analysis_embed - neighbor_analysis_embed, p=2).view(-1)

                # Agreement Signals (Dot product of normalized vectors = Cosine Similarity)
                ...
                # Isolated node padding
                neighbor_solution = torch.zeros_like(solution)
                neighbor_analysis_embed = torch.zeros_like(analysis_embed)
            ...
            # --- 5. CONCATENATION (27-DIM MAPPING) ---
            feature_vec = torch.cat([
                solution,                # [5] Choices
                analysis_embed,          # [384] Semantic State
                reliable_scores,         # [1] Trust
                neighbor_solution,            # [5] Crowd Answer
                neighbor_analysis_embed,            # [384] Crowd Logic
                sol_agree,               # [1] Agreement
                ana_agree,               # [1] Semantic Agreement
                sol_disp,                # [1] Decision Conflict
                ana_disp,                # [1] Logical Conflict
                sol_per,                 # [1] Choice Persistence
                ana_per,                 # [1] Reasoning Rigidity
                sol_dev_norm,            # [1] Choice Outlier Score
                ana_dev_norm,            # [1] Reasoning Outlier Score
                has_neighbor,            # [1] Topology
            ], dim=0)
            
            node_features.append(feature_vec)
        
        node_features = torch.stack(node_features, dim=0)
        rnn_hidden_states = self._stack_h_states(rnn_hidden_states)
        
        return node_features, rnn_hidden_states

    def embed_messages_from_neighbors(self, neighbors_node_ids: List[str]):
        """
        Aggregates neighbor information using weighted normalization.
        """
        n_sols, n_anas, n_scores = [], [], []

        for nid in neighbors_node_ids:
            node = self.find_node(nid)
            n_sols.append(node.last_memory['solution_embed'].view(-1))
            # Normalize neighbors to sphere surface
            raw_ana = node.last_memory['analysis_embed'].view(-1)
            n_anas.append(F.normalize(raw_ana, p=2, dim=0))
            n_scores.append(node.last_memory['reliable_scores'].view(-1))
        
        n_sols = torch.stack(n_sols, dim=0).to(self.device)
        n_anas = torch.stack(n_anas, dim=0).to(self.device)
        raw_scores = torch.stack(n_scores, dim=0).to(self.device)
        
        # Reliability-weighted aggregation
        weights = torch.softmax(raw_scores, dim=0).view(-1, 1)
        
        sol_center = (n_sols * weights).sum(dim=0)
        ana_center_raw = (n_anas * weights).sum(dim=0)
        # Re-normalize center to sphere surface
        ana_center = F.normalize(ana_center_raw, p=2, dim=0)

        # Conflict/Dispersion Metrics
        eps = 1e-9
        y_ent = -torch.sum(sol_center * torch.log(sol_center + eps))
        sol_disp = y_ent / torch.log(torch.tensor(float(self.solution_dim)))

        # ana_disp: Semantic Spread using L2
        # High spread = real debate; Low spread = 4/6 coordination
        dist_to_center = torch.norm(n_anas - ana_center, p=2, dim=1)
        ana_disp = torch.sum(dist_to_center * weights.view(-1)) / 2.0 

        return sol_center, ana_center, sol_disp.view(-1), ana_disp.view(-1)

    def _stack_h_states(self, h_states):
        if h_states is None or any(h is None for h in h_states):
            return None
    ...
    def get_weighted_majority_vote(self, reliable_scores:torch.Tensor, samples: torch.Tensor, training: bool = True):
        ...
        for node_id, node in self.nodes.items():
            ...
            output = node.outputs[-1].strip()
            solution = postprocess_answer(output)
            solution_embed = self.prompt_set.change_solution_to_one_hot_vector(solution, self.solution_dim)
    ...
    async def arun(self, input: Dict[str,str], 
        ...
        # inputs:{'task':"xxx"}
            ...
            elif self.domain == "arc-c":
                # pdb.set_trace()
                # check how many options are there
                num_options = len(input.keys()) - 1
                all_options = [opt for opt in input.keys() if opt not in ["task"]]
                wrong_options = [opt for opt in all_options if opt != correct_answer]
                global_wrong_answer = random.choice(wrong_options)
            elif self.domain == "medqa":
                wrong_options = [opt for opt in ["A", "B", "C", "D", "E"] if opt != correct_answer]
                global_wrong_answer = random.choice(wrong_options)

            num_adv = int(len(self.nodes) * attack_rate)
            adv_indices = list(range(len(self.nodes)))[-num_adv:]
        ...
        for round in range(num_rounds+1):
            if training:
                print(f'--------------------------------{self.domain} {iters} train round {round} --------------------------------')
            else:
                print(f'--------------------------------{self.domain}  eval round {round} --------------------------------')
