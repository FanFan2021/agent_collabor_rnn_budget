...
class Graph(ABC):
    ...
    def construct_node_features(self):
        ...
        for idx in range(len(self.nodes)):
            ...
            mem, first_mem = node.last_memory, node.first_memory
            ...
            # Persistence metrics
            first_sol = first_mem['solution_embed'].to(self.device)
            first_ana = first_mem['analysis_embed'].to(self.device)
            sol_per = torch.norm(solution - first_sol, p=1).view(-1)
            ana_per = torch.norm(analysis_embed - first_ana, p=2).view(-1)

            # Neighbor features
            if mem['neighbors']:
                neighbor_solution, neighbor_analysis_embed, sol_disp, ana_disp = self.embed_messages_from_neighbors(mem['neighbors'])
