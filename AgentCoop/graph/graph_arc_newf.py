...
class Graph(ABC):
    ...
    def construct_node_features(self):
        ...
        for idx in range(len(self.nodes)):
            ...
            if len(neighbors_node_ids) > 0:
                # Weighted centers and conflict signals from embed_messages_from_neighbors
                neighbor_solution, neighbor_analysis_embed, sol_disp, ana_disp = self.embed_messages_from_neighbors(neighbors_node_ids)

                # A. Solution Agreement (Categorical Match): 1.0 if identical argmax
                ...
                # C. Deviation Norms: Quantify distance from weighted crowd
                # Max L1 distance between probability vectors is 2.0 -> map to [0, 1]
                sol_dev_norm = (torch.norm(solution - neighbor_solution, p=1) / 2.0).view(-1)
                # Max L2 distance between unit vectors is 2.0 -> map to [0, 1]
                ana_dev_norm = (torch.norm(analysis_embed - neighbor_analysis_embed, p=2) / 2.0).view(-1)

                # D. Persistence: Comparison with Round 1 (Detecting the "Flip")
                ...
                # Zero Padding for isolated agents
                ...
                sol_per = ana_per = torch.tensor(1.0, device=self.device).view(-1) # Neutral persistence
                has_neighbor = torch.tensor(0.0, device=self.device).view(-1)
            
            # --- 4. CONCATENATION ---
            # Note: In a 27-dim constraint, solution/analysis are usually projected first
            feature_vec = torch.cat([
                solution,                # Current Choice Vector
                analysis_embed,          # Current Logic Vector
                reliable_scores,         # Current Trust Score
                neighbor_solution,       # Group Majority Choice
                neighbor_analysis_embed,  # Group Majority Logic
                sol_agree,               # Choice Consensus Signal
                ana_agree,               # Logic Consensus Signal
                sol_disp,                # Choice Entropy
                ana_disp,                # Logic Divergence
                sol_per,                 # Choice Stability (Lie Detector)
                ana_per,                 # Logic Stability (Drift Detector)
                sol_dev_norm,            # Numerical Choice Distance
                ana_dev_norm,            # Numerical Logic Distance
                has_neighbor             # connectivity flag
            ], dim=0)
    ...
    def embed_messages_from_neighbors(self, neighbors_node_ids: List[str]):
        ...
        for n_id in neighbors_node_ids:
            ...
            # Normalize neighbors before stacking for semantic parity
        ...
        # Softmax weights ensure the most reliable agents dominate the "Majority" center
        weights = torch.softmax(raw_scores, dim=0).view(-1, 1)
        
        # Weighted Consensus Centers
        ...
        # --- DISPERSION (Byzantine Conflict Signals) ---
        # Choice Entropy (MedQA A-E)
        epsilon = 1e-9
    ...
    def get_weighted_majority_vote(self, reliable_scores:torch.Tensor, samples: torch.Tensor, training: bool = True):
        ...
        for node_id, node in self.nodes.items():
            ...
            output = node.outputs[-1].strip()
            solution = postprocess_answer(output)
            solution_embed = change_solution_to_one_hot_vector(solution, self.solution_dim)
