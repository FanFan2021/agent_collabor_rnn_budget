...
class Graph(ABC):
    ...
    def construct_node_features(self):
        ...
        for idx in range(len(self.nodes)):
            node = self.nodes[self.idx_to_node_id[idx]]
            mem = node.last_memory

            # Self features
            solution = mem['solution_embed'].to(self.device).view(-1)
            analysis_embed = mem['analysis_embed'].to(self.device).view(-1)
            reliable_scores = mem['reliable_scores'].detach().to(self.device).view(-1)
            
            # Hidden state
            h_state = mem['rnn_hidden_states']
            if h_state is None:
            ...
            # First memory for persistence
            first_sol = node.first_memory['solution_embed'].to(self.device)
            first_ana = node.first_memory['analysis_embed'].to(self.device)
            sol_per = torch.norm(solution - first_sol, p=1).view(-1)
            ana_per = torch.norm(analysis_embed - first_ana, p=2).view(-1)

            # Neighbor features
            neighbors = mem['neighbors']
            if neighbors:
                neighbor_solution, neighbor_analysis_embed, sol_disp, ana_disp = self.embed_messages_from_neighbors(neighbors)
                ...
                has_neighbor = torch.ones(1, device=self.device)
            else:
                ...
                sol_agree = ana_agree = sol_dev_norm = ana_dev_norm = sol_disp = ana_disp = torch.zeros(1, device=self.device)
                has_neighbor = torch.zeros(1, device=self.device)
    ...
    def embed_messages_from_neighbors(self, neighbors_node_ids: List[str]):
        ...
        # Ensure weights are [N, 1] for broadingcasting
        weights = torch.softmax(raw_scores, dim=0).view(-1, 1)
        
        # 4. Weighted Centers
        neighbor_solution_center = (neighbor_solutions * weights).sum(dim=0)
        neighbor_analysis_embed_center = (neighbor_analysis_embeds * weights).sum(dim=0)

        # REFINEMENT: Re-normalize the center to put it back on the unit sphere
        # neighbor_analysis_embed_center = F.normalize(neighbor_analysis_embed_center_raw, p=2, dim=0)

        # 5. Compute Dispersion (Conflict Signals)
            
        # A. solution_disp: Categorical Entropy (Red Alert for 50/50 splits)
        epsilon = 1e-9
        y_entropy = -torch.sum(neighbor_solution_center * torch.log(neighbor_solution_center + epsilon))
        # solution_disp = y_entropy / torch.log(torch.tensor(4.0)) # Normalized 0 to 1
        solution_disp = y_entropy / torch.log(torch.tensor(float(len(neighbor_solution_center))))

        # B. ana_disp: Semantic Spread (Logical Noise Detector)
        dist = torch.norm(neighbor_analysis_embeds - neighbor_analysis_embed_center, p=2, dim=1)
        ana_disp = torch.sum(dist * weights.view(-1)) / 2.0 # Normalized 0 to 1

        return neighbor_solution_center, neighbor_analysis_embed_center, solution_disp.view(-1), ana_disp.view(-1)
    ...
    async def arun(self, input: Dict[str,str], 
        ...
        # pdb.set_trace()
        ground_truth = torch.ones(num_nodes, device=self.device)
        for adv_idx in adv_indices:
            ground_truth[adv_idx] = 0
        ...
        predict_loss = F.binary_cross_entropy_with_logits(reliable_scores, ground_truth)
        print(f"predict_loss: {predict_loss:.4f}")


        avg_score = reliable_scores.mean()
        predictions = (reliable_scores > avg_score).float() #
        predict_acc = (predictions == ground_truth).float().mean() #
        print(f"predict_acc: {predict_acc:.4f}")
        ...
        # pdb.set_trace()
        return final_answers, torch.stack(comm_log_probs), predict_loss, predict_acc
