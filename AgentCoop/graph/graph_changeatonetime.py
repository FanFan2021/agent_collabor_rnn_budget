...
class Graph(ABC):
    ...
    async def arun(self, input: Dict[str,str], 
        ...
        # Returns a random integer N such that 2 <= N <= 4
        # random: randomly choose attack rounds and randomly choose the number of adversarial agents
        # fixed: use the fixed attack rounds and the fixed number of adversarial agents (adversarial agents also fixed)
        # half: use the fixed attack rounds, randomly choose the number of adversarial agents
        if attack_mode == "random":
            attack_rounds = random.randint(2, 3)
        elif attack_mode == "fixed":
            attack_rounds = attack_rounds
        elif attack_mode == "half":
            attack_rounds = attack_rounds
        else:
            raise ValueError("attack_mode must be 'random' or 'fixed' or 'half'")
