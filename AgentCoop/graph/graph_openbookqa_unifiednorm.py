...
import torch
from dataset.arc_challenge_dataset import postprocess_answer
...
class Graph(ABC):
    ...
    def construct_decision_edges(self, reliable_scores: torch.Tensor, samples: torch.Tensor, training: bool = True, temperature: float = 0.5):
        ...
        for node_id, node in self.nodes.items():
            ...
            solution_embed = self.prompt_set.change_solution_to_one_hot_vector(solution, self.solution_dim).to(self.device)
    ...
    async def arun(self, input: Dict[str,str], 
        ...
        for round in range(num_rounds+1):
            if training:
                print(f'--------------------------------{iters} train round {round} --------------------------------')
            else:
