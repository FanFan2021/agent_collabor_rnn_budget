...
class Graph(ABC):
    ...
    def construct_node_features(self):
        ...
        for idx in range(len(self.nodes)):
            ...
            if len(neighbors_node_ids) > 0:
                ...
                # C. Deviation Norms: Quantify the "Distance" from the crowd
                # Max L2 distance between unit vectors is 2.0 -> normalize to [0, 1]
                ana_dev_norm = (torch.norm(analysis_embed - neighbor_analysis_embed, p=2) / 2.0).view(-1)
                
                # Max L1 distance between probability distributions is 2.0 -> normalize to [0, 1]
                sol_dev_norm = (torch.norm(solution - neighbor_solution, p=1) / 2.0).view(-1)

                # D. Persistence (Round T vs Round 1): The ultimate "Lie Detector"
                # Did the answer flip?
                ...
                # Did the logic drift? Map [-1, 1] -> [0, 1]
                ...
                # Isolated Node Case: Padding with neutral values
                ...
                sol_per = ana_per = torch.tensor(0.0, device=self.device).view(-1) # Stable by default
                has_neighbor = torch.tensor(0.0, device=self.device).view(-1)
            
            # pdb.set_trace()
            
            # --- 3. CONCATENATION ---
            # Note: solution and analysis_embed are often reduced via a projection layer 
            # before concatenation to keep the total vector at 27-dim.
            feature_vec = torch.cat([
                solution,                # Categorical state
                analysis_embed,          # Semantic state
                reliable_scores,         # Trust state
                neighbor_solution,       # Crowd answer
                neighbor_analysis_embed,  # Crowd logic
                sol_agree,               # Behavioral matching
                ana_agree,               # Logic matching
                sol_disp,                # Choice chaos
                ana_disp,                # Logic noise
                sol_per,                 # Choice stability
                ana_per,                 # Logic stability
                sol_dev_norm,            # Answer distance
                ana_dev_norm,            # Logic distance
                has_neighbor             # Graph context flag
            ], dim=0)
            
            node_features.append(feature_vec)

            # RNN Hidden State Management
    ...
    def embed_messages_from_neighbors(self, neighbors_node_ids: List[str]):
        ...
        for n_id in neighbors_node_ids:
            ...
            # IMPORTANT: Normalize each neighbor before stacking to ensure semantic parity
        ...
        # Compute Trust-Weights (Softmax emphasizes reliable agents)
        ...
        # Semantic Center: Sum weighted unit vectors and re-project back to unit sphere
        ...
        # --- COMPUTE DISPERSION (Conflict Signals) ---
            
        # A. solution_disp (Normalized Entropy): [0.0 = total agreement, 1.0 = total chaos]
        epsilon = 1e-9
        y_entropy = -torch.sum(neighbor_solution_center * torch.log(neighbor_solution_center + epsilon))
        # neighbor_solutions.shape[1] is the number of choices (e.g., 5 for MedQA)
        sol_disp = y_entropy / torch.log(torch.tensor(float(neighbor_solutions.shape[1])))

        # B. ana_disp (Semantic Spread): Weighted distance from the center
        # Uses (1.0 - CosineSimilarity) to measure angular divergence
        cos_sims = F.cosine_similarity(neighbor_analysis_embeds, neighbor_analysis_embed_center.unsqueeze(0))
        # Map divergence [0, 2] to [0, 1]
        ana_disp = torch.sum((1.0 - cos_sims) * weights.view(-1)) / 1.0 
