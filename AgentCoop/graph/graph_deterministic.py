...
class Graph(ABC):
    ...
    def construct_comm_connection(self, reliable_scores: torch.Tensor, threshold: float = None, training: bool = True): 
        ...
        if threshold is not None:
            # Compute threshold
            adaptive_threshold = reliable_scores.mean()

            # Vectorized sampling based on threshold
            samples = (reliable_scores > adaptive_threshold).float().to(self.device)

        else:
            dist = torch.distributions.Bernoulli(probs=reliable_scores)
            samples = dist.sample()    # shape [N]

        # Clamp to avoid log(0)
        probs = reliable_scores.clamp(epsilon, 1 - epsilon)

        # Bernoulli log-prob
        log_probs = samples * torch.log(probs) + (1 - samples) * torch.log(1 - probs)
        ...
        for sender_idx_tensor in sorted_idx:
            ...
            for receiver_idx_tensor in sorted_idx:
                ...
                if samples[receiver_idx].item() == 0:
                    continue
        ...
        return log_probs.sum(), adj, samples
    ...
    async def arun(self, input: Dict[str,str], 
        ...
        for round in range(num_rounds):
            ...
            log_prob, adj, samples = self.construct_comm_connection(reliable_scores=reliable_scores, threshold=threshold, training=training)
        ...
        # await self.decision_node.async_execute(input)
        # final_answers = self.decision_node.outputs
        final_answers = self.construct_decision_edges(reliable_scores, samples=samples)
        if len(final_answers) == 0:
            final_answers.append("No answer of the decision node")
